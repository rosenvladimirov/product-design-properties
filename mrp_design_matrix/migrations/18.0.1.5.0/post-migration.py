...
"""
Post-migration script run when upgrading to 18.0.1.5.0.
...
def _backfill_empty_design_params(env):
    Lot = env["stock.lot"]
    # Lots with a definition but no params dict — backfill with {} so
    # downstream .get() calls don't hit NoneType.
    lots = Lot.search(
        [
            ("design_param_definition_id", "!=", False),
            ("design_params", "=", False),
